...
import cn.gaily.pub.trigger.timer.ETLBackgrondDbUtil;
...
public class MgrXflServiceImpl implements IMgrXflService {
	...
	@Override
	public void synHistory(MgrInfoVO vo) {
	...
	public List<String> checkTabs(List<String> tabs) {
		...
		JdbcSession session = null;
		Connection cc = null;
		List<String> validTabs = new ArrayList<String>();
		try {
			session = new JdbcSession();
			cc = session.getConnection();
			boolean has = false;
			for(int i=0;i<tabs.size();i++){
				has =SimpleSession.checkHasTable(cc, tabs.get(i));
				if(has){
					validTabs.add(tabs.get(i));
				}
			}
			SimpleJdbc.releaseConn(cc);
		} catch (DbException e) {
			e.printStackTrace();
		}
